class PlayFairCipher:
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        banco = ""
            # Loaibó ký tự 'X' nếu nó là ký tự cuối cùng và là ký tự được thêm vào
        for i in range(0, len(decrypted_text)-2, 2):
            if decrypted_text[i] == decrypted_text[i+2]:
                banco += decrypted_text[i]
            else:
                banco += decrypted_text[i] + "" + decrypted_text[i+1]

        if decrypted_text[-1] == "X":
            banco += decrypted_text[-2]
        else:
            banco += decrypted_text[-2]
            banco += decrypted_text[-1]
        return banco